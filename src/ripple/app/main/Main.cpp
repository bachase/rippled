    ...
    char** argv)
{
    ...
    if (!child)
    {
        ...
        command_line += " --unittest-child";

        std::string exe_name = argv[0];
        std::vector<std::string> args;
        command_line = command_line.substr(exe_name.size());
        boost::split(args, command_line, boost::is_any_of("\t "));
...
int run (int argc, char** argv)
{
    ...
    ("unittest-child", "Run the process as a unit test child process")
#endif
