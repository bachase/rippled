...
public:
    ...
    */
    ...
    /** Link information between peer1 and peer2.

        @return The link information between peer 1 and peer 2
                or none if no connection exists.
    */
    boost::optional<link_type>
    link(Peer const& peer1, Peer const& peer2);
...
public:
    ...
    public:
        ...
        result_type(
            ...
            Peer const& to_,
            link_type link_)
            : to(to_), link(link_), net_(net), from_(from)
        {
        }
    ...
    {
        return result_type(net_, from_, v.first, v.second);
    }
};
    ...
    duration const& delay)
{
    ...
    if (!links_[from].emplace(to, link_type{false, delay, now}).second)
        return false;
    auto const result = links_[to].emplace(from, link_type{true, delay, now});
    (void)result;
...
template <class Peer>
auto
BasicNetwork<Peer>::link(Peer const& from, Peer const& to)
    -> boost::optional<link_type>
{
