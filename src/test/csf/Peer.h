...
struct Peer
{
    ...
    */
    ...
    {
        return validations.laggards(seq, trustedNodes);
    }

    bool
    already(Ledger::Seq const seq)
    {
        bool const ret = validations.maxBySeq(seq + Ledger::Seq{1}) >= quorum;
        if (ret)
            j.debug() << "consensus already reached for seq " << seq;
        return ret;
    }
